...
def tags_entry():# Эта функция определяет параметры easygui multenterbox и вызывает field_check, если пользователь вводил значнеие, вызывает тест на подобие; если совпадение не найдено, пользователь возвращается окно ввода
    ...
    if field_values != None:
        ...
        similarity_test(user_input)#функция, которая проверяет ввод пользователя на сходство с данными в файле, если пользователь нажимает отмену, то его возвращает обратно на главное меню
...
def similarity_test(user_input):#функция, которая сравнивает ввод пользователя с данными в файлах на предмет совпадения более 90%
    tag_list = tags['tags'].unique()
    query = user_input
    choises = tag_list
    ...
    if similarity == []:#если совпадений не найдено, то возвращаемся обратно в окно ввода
        ...
        tag()
def similarity_test2(user_input2):
